...
backprop nn input output learningRate totalError totalIterations = do
  ...
  let updatedNN = update nn (reverse (gradients (reverse (weights nn)) (reverse (biases nn)) (reverse (init activations)) ((last activations) - output))) learningRate
...
update :: NeuralNetwork -> [(Matrix Float, Matrix Float)] -> Float -> NeuralNetwork
update nn updates learningRate = NeuralNetwork (config nn) (update' (weights nn) (fst (unzip updates)) learningRate) (update' (biases nn) (snd (unzip updates)) learningRate)
